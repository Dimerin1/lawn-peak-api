...
def home():
    ...
    return jsonify({
        ...
        'version': '1.0',
        'routes': [str(rule) for rule in api_bp.url_map.iter_rules()]
    })
...
@api_bp.route('/create-setup-intent', methods=['POST'])
...
@api_bp.route('/charge-customer', methods=['POST'])
def charge_customer():
    try:
        data = request.json
...
@api_bp.route('/api/lot-size', methods=['POST'])
...
@api_bp.route('/test-stripe-account', methods=['GET'])
...
@api_bp.route('/create-test-customer', methods=['GET'])
